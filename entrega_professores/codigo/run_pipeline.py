...
Ao final, a pipeline esvazia o bucket e destroi a stack CloudFormation.
...
from src.cloud.aws_runtime import provision_s3_bucket, destroy_s3_stack
...
def main():
    ...
    try:
        ...
        if bucket:
            print("\n>>> Limpeza obrigatoria AWS (esvaziar bucket + destruir stack)")
            ini = time.time()
            destroy_s3_stack(bucket)
            print(f"    (ok em {time.time()-ini:.1f}s)")
