...
def run():
    ...
    cand={k:r[k] for k in ["knn_hardcode","knn_sklearn","mlp"]}; best=max(cand,key=lambda k:cand[k]["f1"])
    smap={"knn_hardcode":p_hc,"knn_sklearn":p_sk,"mlp":p_mlp}
    pmap={"knn_hardcode":y_hc,"knn_sklearn":y_sk,"mlp":y_mlp}
    ...
    for nm,sc in [("KNN (na mão)",p_hc),("KNN (sklearn)",p_sk),("MLP",p_mlp)]:
    ...
    out={"tarefa":"classificacao binaria (sla_breach)","algoritmos":"KNN (hard-code + sklearn), MLP",
